...
class Vertex:
    ...
    def addPath(self, nbr, weight=INFINITY):
        try:
            if self.connectedTo[nbr][0] > weight:
    ...
    E.g.:
        ...
        print(g.getVertex('V0').connectedTo) gives {'V1':[1],'V2':[2],'V3':[2,['V1']],'V4':[3,['V3','V1']],'V5':[3,['V2']]}
...
E.g.:
    ...
    def getDistance(self, start=None, to=None):
        if (start.id in self.vertList) and (to.id in self.vertList):
            return self.vertList[start.id].getWeight(to.id)
        else:
            return None
    ...
    def setDistance(self, start=None, to=None, dist=INFINITY):
        ...
        """
        if to.id in self.vertList[start.id].connectedTo.keys():
            self.vertList[start.id].connectedTo[to.id][0] = dist
        else:
            self.vertList[start.id].connectedTo[to.id] = [dist]
    ...
    def setPred(self, start=None, to=None, pred=None):
        ...
        """
        if to.id in self.vertList[start.id].connectedTo.keys():
            if len(self.vertList[start.id].connectedTo[to.id]) > 1:
                self.vertList[start.id].connectedTo[to.id][1].append(pred)
            else:
                self.vertList[start.id].connectedTo[to.id].append([pred])
                print(len(self.vertList[start.id].connectedTo[to.id]))
        else:
            self.setDistance(start, to)
            start.connecetedTo[to.id].append([pred])
    ...
    def dijkstra(self, start):
        ...
        queue = []
        self.populatePaths()
        for x, v in self.vertList.items():
            if x != start.id:
                queue.append([start.connectedTo[x][0], v])
        pq.heapInsert(queue)
        while not pq.isEmpty():
            current = pq.popMin()
            vertex = current[1]
            for nextVert in self.vertList[vertex.id].getConnections():
                if nextVert != vertex.id:
                    nextVertex = self.vertList[nextVert]
                    currentDist = self.getDistance(start, vertex)
                    newDist = currentDist + self.vertList[vertex.id].getWeight(nextVertex.id)
                    if newDist < self.getDistance(start, nextVertex):
                        self.setDistance(start, nextVertex, newDist)
                        self.setPred(start, nextVertex, vertex)
    ...
    def betweenness_func(self):
        ...
        # print(self.vertList['V1'].connectedTo)
        for k1, v1 in self.vertList.items():
            self.dijkstra(self.getVertex(k1))
            # for k2 in v1.connectedTo.keys():
            #     print(k2)
            #       if k1 != k2:
            #     if len(v1.connectedTo[k2]) > 1:
            for i in v1.getPreds():
                # print("Test", type(i))
                if v1 != i:
                    if i.id in dictionary_of_betweenness.keys():
                        dictionary_of_betweenness[i.id] += 1
                    else:
                        dictionary_of_betweenness[i.id] = 1
        max = 0
        for k,v in dictionary_of_betweenness.items():
            if v > max:
                max = v
                max_bc_node_id = k
        print(dictionary_of_betweenness)
        return max_bc_node_id, dictionary_of_betweenness
